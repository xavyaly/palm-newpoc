   ...
   if (move_uploaded_file($file_tmp, $target_file)) {
	  ...
	  if(strpos($output,"HAND DETECTED")>=1){
        ...
        if(strpos($output,"PALM MATCHED")>=1){
		   $res->sendResponse("ALLREADY REGISTERED",$target_file,1);
        } else {
			copy("./100001_RGB_PALM.jpg","./rgb/".uniqid("P")."".rand(100,999).".jpg");
			$res->sendResponse("SUCCESSFULLY REGISTERED",$target_file,0);
		}
		  
	  }
      else {
		file_put_contents($log_filename, "PALM NOT DETECTED" . "", FILE_APPEND);
